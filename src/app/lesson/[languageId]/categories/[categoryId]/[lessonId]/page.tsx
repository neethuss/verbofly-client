...
const VideoLessonPage = () => {
  ...
  useEffect(() => {
    const fetchLessonData = async () => {
      ...
      try {
        ...
        const isLessonCompleted = progressData.data.languages.some((lang: any) =>
          lang.lessons.some((les: any) => les.lesson && les.lesson._id && les.lesson._id === lessonId && les.isCompleted)
        );
  ...
  const handleComplete = async () => {
    try {
      ...
      await updateProgress(token as string, userId, languageId as string, lessonId as string, true);
      
      setIsCompleted(true);
      toast.success("Lesson completed successfully!");
    } catch (error) {
      console.error("Error updating progress:", error);
      toast.error("An error occurred while updating progress. Please try again later.");
    }
  };
